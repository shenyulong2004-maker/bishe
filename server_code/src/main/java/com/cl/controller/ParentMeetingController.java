...
public class ParentMeetingController {
    ...
    /** 家长发起一对一预约 */
    @PostMapping("/book")
    public R parentBook(@RequestBody ParentMeetingEntity body, HttpServletRequest request) {
        Long userId = (Long) request.getSession().getAttribute("userId");
        String role = (String) request.getSession().getAttribute("role");
        String tableName = (String) request.getSession().getAttribute("tableName");
        if (!isParent(role, tableName) || userId == null) return R.error(403, "仅家长可预约");

        Map<String, Object> result = parentMeetingService.parentBookOneToOne(userId, body);
    ...
    public R confirm(@PathVariable("id") Long id, HttpServletRequest request) {
        ...
        String role = (String) request.getSession().getAttribute("role");
        String tableName = (String) request.getSession().getAttribute("tableName");
        if (!isParent(role, tableName) || userId == null) return R.error(403, "仅家长可确认");

        Map<String, Object> result = parentMeetingService.confirmByParent(userId, id);
        if (!(Boolean) result.get("ok")) return R.error(String.valueOf(result.get("msg")));
        return R.ok();
    }

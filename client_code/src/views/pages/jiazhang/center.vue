					...
					<el-row>
						...
						<el-col :span="12">
							<el-form-item prop="xingming" label="姓名">
								<el-input class="list_inp" v-model="userForm.xingming" placeholder="姓名" readonly></el-input>
							</el-form-item>
						</el-col>
					</el-row>
...
<script setup>
	...
	const xuehaoChange=()=>{
		context?.$http({
			url: `follow/xuesheng/xuehao?columnValue=` + userForm.value.xuehao,
			method: 'get'
		}).then(res=>{
			//带出姓名字段
			if(res.data.data.xingming){
				userForm.value.xingming = res.data.data.xingming
			}
		})
	}
	//菜单悬浮的显示与隐藏
